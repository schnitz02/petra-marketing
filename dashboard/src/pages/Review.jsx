  ...
  return (
    <div className="space-y-14 animate-fade-in">
      ...
      <header className="animate-fade-in-up">
        <p className="label-upper mb-3">Daily Review</p>
        <h1 className="text-[3rem] lg:text-[3.5rem] tracking-tight leading-none">Review</h1>
        <div className="warm-divider mt-5 max-w-[120px]" />
      </header>

      {/* Date Navigation */}
      <div className="flex items-center justify-center gap-5 animate-fade-in-up delay-1">
        <button
          onClick={() => navigateReview(1)}
          disabled={historyIdx >= history.length - 1}
          className="w-10 h-10 rounded-xl border border-[#DDD7CE] bg-[#FDFBF7] flex items-center justify-center hover:border-[#C8956C] hover:text-[#C8956C] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
        >
      ...
      {/* Score Card */}
      <div className="slab p-8 animate-fade-in-up delay-2">
        <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-10">
          <div className="flex flex-col items-center">
            <p className="text-[11px] font-bold text-[#8B8680] uppercase tracking-[0.15em] mb-2">Score</p>
      ...
      {perf && (
        <div className="animate-fade-in-up delay-3">
          ...
          <div className="slab p-8 space-y-6">
            <p className="text-[15px] text-[#2C2420] leading-relaxed">{perf.summary}</p>
        ...
        <div className="animate-fade-in-up delay-4">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              {competitors.summary && (
                <p className="text-[13px] text-[#8B8680] mt-0.5">{competitors.summary}</p>
              )}
            </div>
          </div>
            ...
            {competitors.competitors.map((comp, i) => {
              ...
              return (
                <div key={i} className="slab p-7" style={{ borderLeft: '3px solid #D4A76A' }}>
                  ...
                  <p className="text-[14px] text-[#5F6360] leading-relaxed">{comp.activity}</p>
                </div>
              )
            })}
          </div>
          ...
          <div className="slab p-8">
            <ul className="space-y-4">
              {takeaways.items.map((item, i) => (
                <li key={i} className="flex items-start gap-3.5 text-[15px] text-[#2C2420]">
                  <span className="w-7 h-7 rounded-full bg-[#C8956C]/10 text-[#C8956C] flex items-center justify-center text-[12px] font-bold shrink-0 mt-0.5">
                    {i + 1}
                  </span>
                ...
                <div key={i} className="slab p-7">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    ...
                    <button
                      ...
                      className="shrink-0 px-4 py-2 rounded-xl text-[13px] font-semibold transition-all disabled:opacity-70"
                      style={{
                  ...
                  {item.context && (
                    <p className="text-[14px] text-[#5F6360] leading-relaxed">{item.context}</p>
                  )}
                </div>
              )
            })}
...
function MetricPill({ label, value, color }) {
  return (
    ...
    >
      ...
      <div>
        <p className="text-[11px] font-bold text-[#8B8680] uppercase tracking-[0.1em]">{label}</p>
...
function EmptyState() {
  return (
    <div className="space-y-14 animate-fade-in">
      ...
      <div className="slab-dark p-16 text-center animate-fade-in-up delay-1">
        <div className="w-16 h-16 mx-auto mb-5 rounded-2xl bg-white/[0.06] flex items-center justify-center">
        ...
        <h3 className="text-xl font-semibold text-[#F2EDE6] mb-2">No Reviews Yet</h3>
